<script lang="ts">
  ...
  const getNewBelongingTemplate = (): BelongingData => {
    const data = new ServiceBelonging(undefined).toData();
    data.quantity = 0;
    data.time = TimeGetter.getTimeNow();

    return data;
  };
  const isBelongingEmpty = (data: BelongingData): boolean => {
    return data.title.trim() === "" && data.quantity <= 0;
  };
    ...
    methods: {
      onReset() {
        const values = U.optArray(this.values).map((value) => {
          const data = new ServiceBelonging(undefined).fromData(value).toData();
          data.time = TimeGetter.getTimeNow();
      ...
      onInput() {
        ...
        if (emptyBelongings.length === 0) {
          ...
          while (emptyBelongings.length > 1) {
            const remove = emptyBelongings.pop() as BelongingData;
            const indexRemove = this.belongings.indexOf(remove);
