<script lang="ts">
  ...
  export default Vue.extend({
    ...
    methods: {
      validateTitle(title: string, belonging: BelongingData) {
        title = title.trim();
        if (title === "") {
      ...
      onTitleInput(event: Event, belonging: BelongingData) {
        const element = event.target as HTMLInputElement;
        belonging.title = this.validateTitle(element.value ?? "", belonging);
        this.$emit("invalidate");
      },
      onTitleChange(event: Event, belonging: BelongingData) {
        const element = event.target as HTMLInputElement;
        this.validateTitle(element.value, belonging);
        this.$emit("invalidate");
      },
      onDescriptionInput(event: Event, belonging: BelongingData) {
        const element = event.target as HTMLTextAreaElement;
        belonging.description = element.value.trim();
      },
      onDescriptionKeyup(event: KeyboardEvent) {
        const element = event.target as HTMLTextAreaElement;
        const { key } = event;
      ...
      onCountChange(event: Event, belonging: BelongingData) {
        const element = event.target as HTMLInputElement;
...
<template>
  <div class="BelongingListEdit-item">
    <input
      ...
      @input="(event) => onTitleInput(event, belonging)"
      @change="(event) => onTitleChange(event, belonging)"
    />
