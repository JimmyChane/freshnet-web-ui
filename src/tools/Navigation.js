export default class Navigation {
    ...
    static VISIBILITIES = Object.values(this.Visibility);
    ...
    setDefaultVisibility(visibility = 0) {
        if (!Navigation.VISIBILITIES.includes(visibility))
            return;
        this.defaultVisibility = visibility;
    }
    setDefaultLayout(layout = 0) {
        if (!Navigation.LAYOUTS.includes(layout))
            return;
    ...
    setVisibility(visibility = 0) {
        ...
        if (request)
            ...
            this.visibilityRequests.push({ page, view, visibility });
    }
    setLayout(layout = 0) {
        if (!Navigation.LAYOUTS.includes(layout))
            return;
        ...
        if (request)
            ...
            this.layoutRequests.push({ page, view, layout });
    }
