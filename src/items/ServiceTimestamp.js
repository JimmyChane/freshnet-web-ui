import { isToday, isYesterday, isThisYear, getDay, getMonth, getYear, previousDay, endOfDay, } from "date-fns";
import U from "@/U";
...
export default class ServiceTimestamp {
    ...
    toCount(strs) {
        return strs.reduce((count, str) => {
            if (textContains("timestamp", str))
                count++;
            if (textContains(this.toString(), str))
                count++;
            return count;
        }, 0);
    }
    toString() {
        if (this.isToday())
            return this.toStringHoursMinutes();
    ...
    toStringHoursMinutes() {
        ...
        return `${hour.toString().length === 1 ? `0${hour}` : hour}:${minute.toString().length === 1 ? `0${minute}` : minute}${afterTime}`;
    }
    isToday() {
