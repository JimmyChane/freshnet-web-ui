      ...
      computed: {
         classDirection() {
            ...
            if (this.direction === Bottom)
               return "ButtonOption2-dropdown-bottom";
            ...
            if (this.direction === TopLeft)
               return "ButtonOption2-dropdown-topLeft";
            if (this.direction === TopRight)
               return "ButtonOption2-dropdown-topRight";
            if (this.direction === BottomLeft)
               return "ButtonOption2-dropdown-bottomLeft";
...
<template>
   <div
      :class="[
         isShowing ? 'ButtonOption2-shown' : '',
         'ButtonOption2',
         'transition',
      ]"
   >
      <button
         class="ButtonOption2-main transition"
      ...
      <div
         class="ButtonOption2-dropdown transition"
         :class="[classDirection]"
         :style="{ display: dropdownDisplay }"
      >
         <button
            class="ButtonOption2-dropdown-item transition"
            v-for="menu in menus"
            :key="menu.key"
            @click="
               () => {
...
<style lang="scss" scoped>
   ...
   .ButtonOption2-shown {
      ...
      .ButtonOption2-dropdown-top {
         transform: translateX(0) translateY(calc(-64% + var(--size-dropdown)))
            scale(1);
      }
      .ButtonOption2-dropdown-bottom {
         transform: translateX(0) translateY(calc(58% + var(--size-dropdown)))
            scale(1);
      }
      .ButtonOption2-dropdown-left {
         transform: translateX(calc(-52% - var(--size-dropdown))) translateY(0)
            scale(1);
      }
      .ButtonOption2-dropdown-right {
         transform: translateX(calc(62% - var(--size-dropdown))) translateY()
            scale(1);
      }
